...
系统、软件、清理、Office、语音、浏览器、Agent 或 Broker 业务能力，只保留本地状态、审计元数据、
设置和诊断导出。它不是“管理员模式”，也不会恢复或重放未完成操作。

在设置页点击“预览并导出已脱敏诊断包”，选择一个尚不存在的本地 ZIP 文件。先检查将包含的两个
成员、大小、目标和明确排除项；确认框默认选择“否”。确认后只在本地创建并验证文件，不会上传。
诊断包恢复等级为 MANUAL：不需要时请自行删除。分享前仍应亲自打开检查，不要把真实密码、Token、
私人文档或音频放进错误描述。完整数据范围见[隐私模型](privacy.md)。

普通日志位于应用本地数据目录的 `logs` 子目录，采用限量轮转；崩溃报告位于 `crash-reports`，只
保留最新十份且不会自动上传。数据库或迁移检查失败时不要删除 `state.db`、降低规则或用管理员身份
绕过；保留错误码和诊断包进行排查。

## 主页与长任务

在“主页与长任务”选择一个安全模板并创建计划。此时不会执行任何业务操作。打开“任务中心”，选中
一个任务并逐任务审查计划；该对话框只确认高层只读协调。随后应用可能打开文件、系统、启动项、
服务、软件、Office 或浏览器页面，你仍需在该页面检查具体对象、风险、Preview、回滚能力并完成其
独立确认。

“暂停”只停止后续协调；“恢复”会先要求 Fresh 检查；“停止”不是撤销，也不会关闭外部卸载器；
“检查中断任务”只核对已有证据，不重放。托盘消息只能带你回到任务中心。任务汇总中的“已验证”
必须来自业务域验证结果，窗口关闭或程序退出不等于成功。

## 使用任务中心与 Memory

每个可行动的聊天/语音请求会在“任务中心”显示任务 ID、状态、节点计数和参与角色，不显示内部
Prompt、思维过程、网页/文档正文或完整聊天。点“取消未来工作”只阻止该任务后续协调，不会撤销已经
由某个业务页完成的操作；恢复文件或系统状态仍使用原业务页的 Undo/手工恢复说明。

“Memory”页面只保存有限偏好。选择 key、输入值并保存时会出现具体确认；可选中一项载入修改或
删除，也可确认后清空一个 scope/全部内容。关闭“允许 Agent 使用和保存 Memory”会停止读取和新
写入，但仍可查看和删除旧值。不要在值框输入密码、API key、Cookie、MFA 或文档正文；应用会阻止
常见 secret 模式，但这不是完整 DLP。

保存的目录只是 UUID reference，recent reference 也只是短时提示。它们不会自动授权扫描或修改。
每个业务页仍会重新选择当前对象、展示 Preview 并按 R0/R1/R2/R3 规则确认。

## 使用 Stage 5C 浏览器

首次安装依赖后运行 `uv run playwright install chromium`。启动应用并打开“浏览器”：先启动临时
会话，输入明确目标和公开 URL，生成计划，核对 Origin、动作、风险和影响后再确认、执行。页面变化
后旧计划不能再用。元素表只提供有限语义动作；购买、付款、预订、发消息、发帖、账户、条款和
私网地址会被阻止，不要尝试用模糊名称绕过。

“用户接管”会让所有待处理确认失效。手工操作后点“交还 Agent”，页面会重新观察，再重新生成
计划。临时会话不导入或保存你的日常浏览器 Cookie/密码；V1 对 SSO、验证码、弹窗和复杂多标签
网站不作保证。不要在测试中使用真实密码、付款信息或内部系统。

下载前选择空目标目录并核对 R1 Preview：来源、单个文件、类型、大小上限、不覆盖和回滚条件。
支持 PDF/TXT/CSV/JSON/DOCX/XLSX 和常见图片，默认 50 MiB。格式检查不是病毒扫描，下载后仍应
使用 Windows 安全软件和受保护视图。未修改的下载可移动到恢复目录；修改后会拒绝自动回滚。
“交给办公文档”只是建议，Office 页面仍要重新选择、预览和确认。详见
[完整模型](browser-automation-model.md) 和 [手工清单](manual-testing/browser-automation.md)。

## 使用 Stage 5B 语音

1. 在 Windows“隐私和安全性 → 麦克风”允许桌面应用访问。应用本身不会更改这项系统设置。
2. 在程序下方的“语音设置”选择 `openai`。密钥使用 `OPENAI_API_KEY` 环境变量；不要发到聊天中。
3. 按住“按住说话”，松开结束。也可用 Tab 聚焦按钮，按住空格说话；没有全局快捷键。
4. 点“查看音频外发确认”，核对供应商、模型、地址、秒数和字节。拒绝则不上传。
5. 识别后检查文字，尤其是名称、路径、数字和“删除/不要删除”，可编辑后提交。
6. 原业务页仍要选择精确对象、查看 Preview、确认计划及即时确认。语音不能替代这些步骤。

“停止语音”只停止录音/网络等待/播报；说“取消”会请求当前关联业务停止后续工作，不是撤销。
语音期间切换页面会使旧上下文失效；隐藏应用、切到其他应用或退出会丢弃录音，返回后不会自启。
如需听安全摘要，在设置启用 SHORT/NORMAL，再手动点击播报并确认文字外发。旁人可能听见播报。

常见错误：`CONFIGURATION_REQUIRED` 检查环境变量和供应商；`MICROPHONE_UNAVAILABLE` 检查默认设备；
`FORMAT_UNSUPPORTED` 表示当前默认麦克风不支持 V1 的 24kHz 单声道 PCM；`STORAGE_UNAVAILABLE`
应先检查本地数据目录，停止后续操作。请勿强制重试、提升权限或删除数据库来绕过错误。
详见 [模型与限制](voice-interaction-model.md)、[手工验收](voice-manual-tests.md)。

## 使用 Stage 5A 办公文档

启动后打开“办公文档”：选择文件并确认只读计划 → 勾选输入 → 独立选择输出 → 添加明确编辑 →
生成差异预览 → 按页面要求备份、确认和保存。默认 `SAVE_AS` 不覆盖。先用合成测试文档练习。
编辑器里的 `body` 是完整文本块，`p:0` 是第一个 Word 段落，`s:0:A1` 是第一张表 A1。
`REPLACE_TEXT` 替换整个目标文本块；`SET_SPREADSHEET_CELL` 修改一个单元格；
`SCALE_NUMBER` 用明确倍率计算数值并保留两位小数（四舍六入五成双）。公式不会自动重算。

原地编辑 / 恢复先点击“创建已验证备份”，再“确认编辑计划”，最后“即时确认并保存”。
在“历史与恢复”选择原事务，重新读取并勾选当前文件，再选择同一路径，才能准备独立恢复。
如果系统提示身份改变、文件占用、备份不完整或目标冲突，请停止并检查；不要强制继续。
取消后仍应查看历史中的实际状态与保留文件路径，尤其是提交已经开始的事务。

模型建议是可选项：在“模型建议”逐段勾选内容，先查看完整发送预览和供应商，再决定是否发送。
建议没有执行权限；默认不自动采用模型编辑，不自动上传全文。日志在原“审计”页面查看，
Office 日志不包含文档正文。具体格式、恢复条件和资源设置见 [Office 使用说明](office-automation-model.md)。

## 使用 Stage 4E3 逐项复查建议

1. 在“系统优化分析”生成、确认并运行只读计划。在“建议”表逐项勾选想复查的建议；默认不选中。
2. 点击“创建复查清单”，再点击“复查下一条”。清单只是待办记录，不会批准批量操作。
   “复查入口风险”中的 R0 仅表示导航复查；后续操作可能是 R2/R3，必须由原业务重新评估和确认。
3. 新页面仍要求读取当前清单、选择对象并查看原业务 Preview。启动项、进程、软件和清理分别确认，
   服务建议只能查看。旧对象、受保护对象、权限不足或身份变化不会由优化层自动修复。
4. 需要写操作时，仔细核对原业务的对象、数量、风险和恢复能力，再决定是否确认。卸载软件不自动
   清理残留；清空回收站是独立的不可恢复操作，不能与普通清理合并确认。
5. 返回清单查看业务结果。“清单完成”不等于“全部执行成功”；关闭窗口或退出码也不能证明成功。
   特权动作和后续 Force/Restore 的详细结果请查看原业务日志，不会冒充 E3 已验证结果。
6. “取消剩余复查”只停止后续项目，不撤销已完成动作，不终止外部卸载器。应用重启后不会自动继续。
7. 可生成独立只读刷新计划并再次确认；缺少可靠前后采样时显示“未测量”。观察值变化不等于因果收益。

遇到无法复查，先检查报告是否过期、原授权目录是否仍有效，再生成新只读报告。数据库或审计错误时
停止操作，保留日志排查；不要删除数据库、放宽保护规则或用管理员运行整个应用来绕过问题。
完整入口表及限制见 [Stage 4E3 说明](optimization-action-routing.md)。

## 使用 Stage 4E2 受控系统清理

1. 先在“系统优化分析”完成新的 Stage 4E1 只读报告。报告候选不会执行任何操作。
2. 在“清理候选”表中勾选想进一步评估的来源，点击“对勾选候选进行 Fresh 安全评估”。
3. 等待应用重新扫描。Fresh 表默认全部不勾选，且没有“全选”；请逐项查看路径、类别、保护、
   文件/目录数、大小、恢复和阻止原因，只勾选 `ELIGIBLE` 对象。
4. 点击“生成所选对象的受控计划”，核对对象数、内部对象数、总大小、R2/R2_HIGH_IMPACT 和
   `MANUAL` 恢复。第一次确认后只会再次检查，不会立即移动。
5. 阅读第二次即时确认。只有点击“立即移入 Windows 回收站”才会逐项执行；默认仍是取消。
6. 结果页区分成功、失败、变化后阻止和未执行。取消或某项失败会停止未来项目，不会假装撤销
   已完成项。成功项需要在 Windows 回收站中手动还原。

直接 V1 范围只有足够旧且通过所有安全检查的当前用户 Temp、DirectX shader cache 和 CrashDumps
精确子对象。个人大文件/闲置/重复候选会提示使用 Stage 2B，软件残留会提示使用 Stage 4D4。
数据库、配置、用户数据、近期/锁定对象、reparse/junction/symlink、其他用户、共享/网络路径、
浏览器缓存和 Windows 管理目录不会因点击确认而放行。没有永久删除或“强制继续”。

移入回收站不等于释放空间，结果中的“已验证释放磁盘空间”会保持未知。如果确实要清空，点击
“独立检查并清空回收站”，核对当前用户系统盘回收站的数量和大小，再分别完成计划确认与即时
确认。此操作固定为 `R2_HIGH_IMPACT`、恢复 `NONE`；清空后 Agent 无法还原。内容在确认期间变化
会使操作失效。测试环境不会替用户清空真实回收站。

## 使用 Stage 4E1 系统优化分析

1. 打开“系统优化分析”，输入目标，例如“释放磁盘空间并诊断电脑变慢”。
2. 如需分析个人文件，可勾选已经在“常用目录”中明确授权的目录；不勾选时不会扫描个人目录。
3. 点击“生成只读计划”，检查工具、范围、对象/时间上限、R0 风险和“系统修改 0”。
4. 点击“确认 R0 计划”。确认只允许本次分析，不允许清理。
5. 点击“开始只读分析”。长任务可取消；权限不足或来源缺失会显示为部分/不可用。
6. 在概览、存储、清理候选、启动项、进程、性能发现和建议页查看证据与置信度。
7. 如需保存结果，可导出到一个尚不存在的 `.json` 或 `.csv` 文件；应用不会覆盖原文件。

计划会按目标缩小采集范围：“C 盘为什么满了”不会额外采集 CPU/服务，“电脑为什么卡”不会扫描
缓存，“开机慢”只关注进程与启动项证据；只有“全面检查”使用完整只读来源。

“潜在空间”不是释放承诺，“疑似闲置”不是无用文件，“候选”不是删除许可。Windows Update、
Delivery Optimization 或 Installer Cache 显示不可用/受保护时，请勿手工删除其目录。Stage 4E1
自身没有一键清理、Boost、Fix 或 Apply；后续 Stage 4E2 必须进入上述独立 Fresh 与双重确认流程。

## 使用 Stage 4X3 专用管理员操作

主程序始终保持普通用户。对于安全策略已允许、但确实缺少管理员权限的单个对象，界面会打开
“Stage 4X3 安全操作”窗口：先显示对象、动作、R3 风险、权限和真实回滚等级；第一次点击只确认
计划。应用重新读取目标后才显示即时确认，第二次点击才会弹出一次 Windows UAC。

- 服务启动类型只支持普通第三方服务的 Automatic 与 Manual 互换；不会启停服务。恢复必须从
  Agent 的恢复历史发起，外部改动会导致冲突停止。
- 启动项页可对严格合格的单个 HKLM Run 项进入管理员 Preview；32/64 位视图、原始值和备份均被
  绑定。恢复不会覆盖新值。
- 软件页在精确识别机器范围 MSI 后进入管理员卸载流程。共享运行库、驱动、安全/网络、Windows、
  Agent、企业和未知软件仍然阻止。机器 MSI 无自动回滚。

在 UAC 选择“否”是安全取消，应用不会重复弹窗。若结果显示“不确定”或“主程序验证失败”，请先
点击只读刷新核对状态，不要立即重试。开发构建只有在 Broker 路径与哈希正确配置时可用；正式版
还要求可信安装与发布签名。当前不支持机器范围 Vendor Uninstaller。
